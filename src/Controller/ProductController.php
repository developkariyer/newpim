...
class ProductController extends AbstractController
{
    ...
    private function validateCsrfTokenAjax(Request $request): bool
    {
        $token = $request->headers->get('X-CSRF-TOKEN') ?? $request->request->get('_token');
        if (!$token) {
            error_log('Security: AJAX CSRF token missing');
            return false;
        }
        $csrfToken = new CsrfToken(self::CSRF_TOKEN_ID, $token);
        $isValid = $this->csrfTokenManager->isTokenValid($csrfToken);
        if (!$isValid) {
            error_log('Security: AJAX CSRF token validation failed');
        }
        return $isValid;
    }

    private function validateRequestSecurity(Request $request): bool
    {
        $contentLength = $request->headers->get('Content-Length');
        if ($contentLength && $contentLength > self::MAX_REQUEST_SIZE) {
            error_log('Security: Request too large: ' . $contentLength . ' bytes');
            return false;
        }
        if ($request->isMethod('POST')) {
            ...
            if (!$isValidContentType) {
                error_log('Security: Invalid Content-Type: ' . $contentType);
                return false;
            }
        }
        if (!empty(self::ALLOWED_ORIGINS)) {
            ...
            if ($origin && !in_array($origin, self::ALLOWED_ORIGINS)) {
                error_log('Security: Invalid Origin: ' . $origin);
                return false;
            }
        }
        $userAgent = $request->headers->get('User-Agent');
        if (empty($userAgent) || strlen($userAgent) < 10) {
            error_log('Security: Suspicious User-Agent: ' . $userAgent);
            return false;
        }
        return true;
    }
